...
void setAttributeNamed(xmlNode * node, const char * nameStr, const char * value) {
	...
	for(xmlAttrPtr attr = node->properties; NULL != attr; attr = attr->next)
	{
		...
		{				
			...
			{
                NSLog(@"old attribute value = %@",[NSString stringWithCString:(void*)child->content encoding:NSUTF8StringEncoding]);
				free(child->content);
				child->content = (xmlChar*)newVal;
                NSLog(@"new attribute value = %@",[NSString stringWithCString:(void*)child->content encoding:NSUTF8StringEncoding]);
				break;
			}
			break;
		}
...
-(void)setAttributeName:(NSString*)name WithValue:(NSString*)value
{
